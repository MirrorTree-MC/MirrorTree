...
const green = chalk.green || chalk.default.green;
const magenta = chalk.magenta || chalk.default.magenta;
const red = chalk.red || chalk.default.red;
...
console.log(table(list.map(({ file, background, foreground }) => {
    return [
        green(file.replace(basename, '')),
        magenta(background.trim()),
        red(foreground.trim())
    ];
})));
